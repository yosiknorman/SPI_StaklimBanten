...
SOFT = readxl::read_excel(paste0("../data/BMKGSOFT/Data Hujan Desember DKI dan Banten_",
                                 InitTime, ".xls"))
load(paste0("../data/CH_SPILast/InputSPI_",LastTime,".Rda"))
...
for(i in 1:ncol(BackUpdataDF)){
  ...
  if(length(which(is.na(BackUpdataDF[, i])))/length(BackUpdataDF[, i]) > 0.6){
    ...
    DFNew = data.frame(year = as.integer(substr(YMDNew, 1, 4)), 
                       month = as.integer(substr(YMDNew, 6, 7)), precipitation = JumlahData[i])
    ...
    png(filename = paste0("../output/Plot/Plot_Sta_",NAMESPI[i], ".png"), 
...
write.csv2(Hasil1BulanLastDF, file = paste0("../output/Hasil1BulanLast_", 
...
InputSPI = InputSPI2
save(InputSPI, file = paste0("../data/CH_SPILast/InputSPI_",InitTime, ".Rda" ))
